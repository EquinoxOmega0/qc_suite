...
if __name__ == "__main__":
    ...
    print "------------------------------------------------"
    print "program qc_parser started"
    ...
    try:
        print "Reading from %s" % (QC_SUITE_DIR+"instrument/"+instrument+"2.dat",)
    ...
    try:
	 ...
	 print "File %s does not exist. Creating it with proper headers." % (outfileName,)
	 outfile = open(outfileName, "a")
    ...
    outfile.close()
    print "program qc_parser terminated"
    print "------------------------------------------------"
    sys.exit(0)
