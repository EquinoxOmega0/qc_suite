...
print "------------------------------------------------"
print "program qc_fetch started"
...
if filestr.split('\n')[6]=='2': #just fetch the two newest files
  ...
  if todayexists:
    ...
    print "fetching "+todaysfile
  else:
     print "WARNING: today\'s file("+todaysfile+") does not exit"
  if yesterdayexists:
    ftp.get(pathtologs+'/'+yesterdaysfile, targetfolder+'/'+yesterdaysfile)
    print "fetching "+yesterdaysfile
  else:
     print "WARNING: yesterday\'s file("+yesterdaysfile+") does not exit"
  ftp.close()
else:
...
print "program qc_fetch terminated"
print "------------------------------------------------"
