...
if __name__ == "__main__":
    ...
    todayslogfile.write(now+"program qc_plotter started\n")
    todayslogfile.close()
    instrument = 'wfi'
    qc_par  = "QC.ZEROPOINT"
    xfield = "MJD-OBS"
 ...
 # read from an instrument configuration file
    ...
    if linesList[0][0] != '#':
        ...
        todayslogfile = open(tlfname, 'a' )
        now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
        todayslogfile.write(now+"File %s does not contain a header line.\n" % (infileName,))
        todayslogfile.close()  
    else:
    ...
    todayslogfile = open(tlfname, 'a' )
    now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
    todayslogfile.write(now+"Data range: %f - %f\n" % (data_min_date, data_max_date,)  )
    todayslogfile.close()  
    ...
    if icount > 0 :
       ...
       todayslogfile = open(tlfname, 'a' )
       now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
       todayslogfile.write(now+"plotting data for "+qc_par+"\n")
       todayslogfile.close()  
	  ...
	  date_start = jd_start - julian_date('1','1','1','0','0','0')
       ...
       todayslogfile = open(tlfname, 'a' )
       now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
       todayslogfile.write(now+plotter_timestamp_str+"\n")
       todayslogfile.close()  
	       ...
	       if len(hioutliers_x) > 0:
			...
			for ii in range(len(hioutliers_x)):	
			   if ignoredoutlier[ii] == 0:
			      ...
			      todayslogfile = open(tlfname, 'a' )
			      now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
			      todayslogfile.write(now+"HIGH OUTLIERS in chip %s\n" % (find_key(chipIndex,i)[0],))
			      todayslogfile.close()  
		              hicolor = 'pink'
                     else :
		        tlfname="QC_suite_"+time.strftime("%Y-%m-%d", time.localtime())+".log"
	       ...
	       if len(lowoutliers_x) > 0:
			...
			for ii in range(len(lowoutliers_x)):	
			   if ignoredoutlier[ii] == 0:
			      todayslogfile = open(tlfname, 'a' )
			      now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
			      todayslogfile.write(now+"LOW OUTLIERS in chip %s\n" % (find_key(chipIndex,i)[0],))
			      todayslogfile.close()  
		              lowcolor = 'pink'
                     else :
		       	todayslogfile = open(tlfname, 'a' )
    ...
    # show the plot if the parameter in the configuration file isn't 1
	  ...
	  plt.show()
       ...
       todayslogfile = open(tlfname, 'a' )
       now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
       todayslogfile.write(now+"not enough data points to plot "+qc_par+"\n")
       todayslogfile.close()  
    ...
    todayslogfile = open(tlfname, 'a' )
    now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
    todayslogfile.write(now+"program qc_plotter terminated\n")
    todayslogfile.write(now+"------------------------------------------------\n")
    todayslogfile.close()
