...
todayslogfile.write(now+"start running the qc suite\n")
todayslogfile.close()
...
skip_prepare = 'F'
skip_sync = 'F'
QC_SUITE_DIR = '/home/equinoxomega/qc_suite_2012-11-05/'

tlfname="QC_suite_"+time.strftime("%Y-%m-%d", time.localtime())+".log"
todayslogfile = open(tlfname, 'a' )
now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
todayslogfile.write(now+"QC_SUITE_DIR: %s\n" % (QC_SUITE_DIR,))
todayslogfile.close()
print 
...
if instrument == "wfi":
  ...
  if instrument == "vimos":
	...
	todayslogfile = open(tlfname, 'a' )
	now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
	todayslogfile.write(now+"unknown instrument: %s\n" %instrument)
	todayslogfile.close()
...
if skip_fetch == 'F':
  ...
  todayslogfile = open(tlfname, 'a' )
  now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
  todayslogfile.write(now+comando+"\n")
  todayslogfile.close()
...
for counter in range(1,nplots+1):
  ...
  todayslogfile = open(tlfname, 'a' )
  now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
  todayslogfile.write(now+"plotting "+str(counter)+" of "+str(nplots)+"\n")
  todayslogfile.close()
  ...
  todayslogfile = open(tlfname, 'a' )
  now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
  todayslogfile.write(now+phstring+"\n")
  todayslogfile.close()
...
todayslogfile = open(tlfname, 'a' )
now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
todayslogfile.write(now+"synchronizing with Garching\n")
todayslogfile.close()
...
for counter in range(1,nplots+1):
  ...
  todayslogfile = open(tlfname, 'a' )
  now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
  todayslogfile.write(now+'executing: cd '+targetfolder+' ; webcp '+plotstr+"\n")
  todayslogfile.close()  
...
todayslogfile = open(tlfname, 'a' )
now=time.strftime("%Y-%m-%dT%H:%M:%S", time.localtime())+" >>> "
todayslogfile.write(now+"running the qc suite is completed\n")
todayslogfile.write(now+"------------------------------------------------\n")
todayslogfile.close()
